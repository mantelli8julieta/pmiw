...
//esto es d chatgpt....
  if (pantallaActual === 0) {
    if (mouseX > 200 && mouseX < 450 && mouseY > 200 && mouseY < 300) {
      pantallaActual = 2; // Iniciar aventura gráfica
    } else if (mouseX > 200 && mouseX < 450 && mouseY > 310 && mouseY < 400) {
      pantallaActual = 1; // Ir a créditos
    }
  } else if (pantallaActual === 10 && !juegoGanado) {
    juegoActivo = true;
    tiempoInicio = millis(); // Iniciar el minijuego
  } else if (pantallaActual === 10 && juegoGanado) {
    if (mouseX > 174 && mouseX < 272 && mouseY > 425 && mouseY < 455) {
      pantallaActual = 11; // Decisión A
    } else if (mouseX > 370 && mouseX < 468 && mouseY > 425 && mouseY < 455) {
      pantallaActual = 12; // Decisión B
    }
  }
}


// esta es la lógica original q escribimos nosotros
/*if (pantallaActual == 0) {
 if (mouseX > 200 && mouseX < 450 && mouseY > 200 && mouseY < 300) {
 pantallaActual = 2;
 //botón créditos
